    ...
    <div style={{ fontFamily: "'Segoe UI', Arial, sans-serif", background: "#080c14", color: "#e8edf5", minHeight: "100vh", overflowX: "hidden" }}>
      ...
      <style>{`
        ...
        @media (max-width: 768px) {
          ...
          .cta-btn, .cta-btn-outline { padding:12px 24px !important; font-size:10px !important; }
        ...
        <div style={{ display:"flex", alignItems:"center", gap:20 }} className="desktop-menu">
          ...
          {user?.rolle === "kunde" && (
            <>
              ...
              <button onClick={handleLogout} style={{ display:"flex", alignItems:"center", gap:6, background:"rgba(162,45,45,0.15)", border:"1px solid rgba(162,45,45,0.3)", color:"#f09595", padding:"7px 12px", borderRadius:50, fontSize:10, fontWeight:700, letterSpacing:"0.1em", textTransform:"uppercase", cursor:"pointer" }}>
                <LogOut size={12} />
              </button>
            </>
          )}
          {user?.rolle === "frisor" && (
            <>
              ...
              <button onClick={handleLogout} style={{ display:"flex", alignItems:"center", gap:6, background:"rgba(162,45,45,0.15)", border:"1px solid rgba(162,45,45,0.3)", color:"#f09595", padding:"7px 12px", borderRadius:50, fontSize:10, fontWeight:700, cursor:"pointer" }}>
                <LogOut size={12} />
              </button>
            </>
          )}
      ...
      <section ref={heroRef} style={{ position:"relative", zIndex:1, minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", padding:"100px 20px 60px", textAlign:"center" }}>
        <div style={{ maxWidth:800, opacity:visible?1:0, animation:visible?"fadeUp 1s ease forwards":"none" }}>
          ...
          <h1 style={{ fontSize:"clamp(32px, 8vw, 88px)", fontWeight:300, lineHeight:1.05, letterSpacing:"-0.02em", marginBottom:20, color:"#e8edf5" }}>
            Dit hår.<br /><span style={{ color:"#378add", fontWeight:700 }}>Din stil.</span>
          </h1>
          <p style={{ fontSize:"clamp(14px, 2vw, 17px)", color:"rgba(232,237,245,0.55)", lineHeight:1.7, maxWidth:520, margin:"0 auto 36px", padding:"0 10px" }}>
            Oplev frisørkunst på højeste niveau. Book din tid online på få sekunder.
          </p>
      ...
      <section style={{ position:"relative", zIndex:1, padding:"20px 20px 60px" }}>
        <div style={{ maxWidth:1100, margin:"0 auto" }}>
          <div style={{ textAlign:"center", marginBottom:40 }}>
            <p style={{ fontSize:10, color:"#378add", fontWeight:700, letterSpacing:"0.3em", textTransform:"uppercase", marginBottom:12 }}>Vores services</p>
            <h2 style={{ fontSize:"clamp(24px, 5vw, 40px)", fontWeight:300, color:"#e8edf5" }}>Hvad kan vi gøre for dig?</h2>
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(200px, 1fr))", gap:16 }}>
            {[{ icon:"✂", title:"Klipning", desc:"Klassisk og moderne klipning." }, { icon:"✨", title:"Styling", desc:"Til hverdag og fest." }].map((s, i) => (
              <div key={i} className="service-card">
                ...
                <h3 style={{ fontSize:15, fontWeight:600, color:"#e8edf5", marginBottom:8 }}>{s.title}</h3>
                <p style={{ fontSize:12, color:"rgba(232,237,245,0.45)", lineHeight:1.6 }}>{s.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ position:"relative", zIndex:1, padding:"0 20px 60px" }}>
        <div style={{ maxWidth:900, margin:"0 auto", background:"rgba(24,95,165,0.12)", border:"1px solid rgba(55,138,221,0.2)", borderRadius:24, padding:"40px 24px", textAlign:"center" }}>
          <h2 style={{ fontSize:"clamp(22px, 4vw, 36px)", fontWeight:300, color:"#e8edf5", marginBottom:12 }}>
