...
export default function CancelPage() {
  ...
  if (loading) {
    return <div style={{ minHeight:"100vh", background:"#080c14", display:"flex", alignItems:"center", justifyContent:"center" }}>
    ...
    <div style={{ minHeight:"100vh", background:"#080c14", color:"#e8edf5", fontFamily:"'Segoe UI', Arial, sans-serif", padding:"20px", display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div style={{ maxWidth:500, width:"100%" }}>
        <div style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(55,138,221,0.12)", borderRadius:20, padding:"32px 24px", textAlign:"center" }}>
          ...
          {result && <div style={{ background:result.success?"rgba(15,110,86,0.1)":"rgba(220,38,38,0.1)", border:`1px solid ${result.success?"rgba(93,202,165,0.2)":"rgba(220,38,38,0.2)"}`, borderRadius:12, padding:"16px", marginBottom:24, color:result.success?"#5dcaa5":"#fca5a5", fontSize:13 }}>{result.message}</div>}
          {bookings.length === 0 && !result && <p style={{ color:"rgba(232,237,245,0.5)", fontSize:13 }}>Ingen kommende bookinger fundet.</p>}
          {bookings.map(b => (
            <div key={b.bookingId} style={{ background:"rgba(255,255,255,0.02)", border:"1px solid rgba(55,138,221,0.08)", borderRadius:14, padding:"16px", marginBottom:12 }}>
              ...
              <p style={{ fontSize:12, color:"rgba(232,237,245,0.5)", marginBottom:12 }}>{new Date(b.startTid).toLocaleString('da-DK',{weekday:'long',day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'})}</p>
              <button onClick={()=>handleCancel(b.bookingId)} disabled={cancelling===b.bookingId} style={{ background:cancelling===b.bookingId?"rgba(220,38,38,0.2)":"rgba(220,38,38,0.1)", border:"1px solid rgba(220,38,38,0.3)", color:"#fca5a5", padding:"10px 24px", borderRadius:8, cursor:cancelling===b.bookingId?"wait":"pointer", fontSize:11, fontWeight:700, letterSpacing:"0.1em", textTransform:"uppercase" }}>{cancelling===b.bookingId?"Aflyser...":"Aflys denne tid"}</button>
            </div>
          ))}
          <Link to="/" style={{ display:"inline-block", marginTop:24, color:"#378add", fontSize:12, textDecoration:"none" }}>← Tilbage til forsiden</Link>
        </div>
      </div>
    </div>
  );
